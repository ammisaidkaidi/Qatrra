...
import { isAuthenticated } from "./global.js";
...
export class System {
    ...
    async load(table, fetch = true, deep) {
        ...
        if (fetch) {
            ...
            const rows = await this.api.fetchAll(table, updated_at);
            if (rows) {
...
export class API extends SystemItem {
    ...
    async updateInvoiceTotal(id) {
        ...
        const { error: updateError } = await this.server
            ...
            .eq('id', id);
        this.getRow('invoices', id).total = newTotal;
        return updateError;
    }
